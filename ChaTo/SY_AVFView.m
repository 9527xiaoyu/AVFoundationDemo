...
@interface SY_AVFView()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate> {
...
@property (strong, nonatomic) SY_RecordEncoder           *recordEncoder;//录制编码
...
- (dispatch_queue_t)captureQueue {
    if (_captureQueue == nil) {
        _captureQueue = dispatch_queue_create("cn.capture", DISPATCH_QUEUE_SERIAL);
    }
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.videoPath = [[self getVideoCachePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"/%@",videoName]];
            self.recordEncoder = [SY_RecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
        //
        // 增加sampleBuffer的引用计时,这样我们可以释放这个或修改这个数据，防止在修改时被释放
        CFRetain(sampleBuffer);
        NSLog(@"%lld",_timeOffset.value);
    ...
    if (self.startTime.value == 0) {
        self.startTime = dur;
        NSLog(@"``%lld",self.startTime.value);
    }
    CMTime sub = CMTimeSubtract(dur, self.startTime);
    self.currentRecordTime = CMTimeGetSeconds(sub);
//    NSLog(@"%f",self.currentRecordTime);
    ...
    if ([self.delegate respondsToSelector:@selector(recordProgress:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate recordProgress:self.currentRecordTime/self.maxRecordTime];
            //每秒录制
            NSLog(@"按秒录制：%f",self.currentRecordTime);
